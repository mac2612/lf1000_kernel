		...
		struct snd_pcm_hw_params *params)
{
	...
	struct lf1000_pcm_dma_params *dma = snd_soc_dai_get_dma_data(soc_runtime->cpu_dai, substream);
	int ret = 0;
...
static int lf1000_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	for (i = 0; i < runtime->periods; i++) {
		addr_list[i] = (u32)virt_to_phys((void *)(runtime->dma_addr + (i*size)));
	}
...
static snd_pcm_uframes_t lf1000_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		ptr = (u32)phys_to_virt(dma_get_write_addr(prtd->dma_ch)) - prtd->dma_buf;
	else
		ptr = (u32)phys_to_virt(dma_get_read_addr(prtd->dma_ch)) - prtd->dma_buf;
